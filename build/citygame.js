...
(function (global, factory) {
	...
	class MiscUtils {
		...
		static rotate10Arrays() {
			for (var i = 0; i < Micro.arrs.length; i++) {
				var name10 = Micro.arrs[i] + 'Hist10'; //this[name10] = [0].concat(this[name10].slice(0, -1));
		...
		static rotate120Arrays() {
			for (var i = 0; i < Micro.arrs.length; i++) {
				var name120 = Micro.arrs[i] + 'Hist120'; //this[name120] = [0].concat(this[name120].slice(0, -1));
		...
		static makeArrayOf(length, value) {
			...
			var result = new Array(length); //for (var a = 0; a < length; a++) result[a] = value;
	...
	class ZoneUtils {
		...
		static HOSPITAL(tile) {} // ZoneUtils
		...
		static fireZone(map, x, y, blockMaps) {
			...
			let zoneSize = 2; // A zone being on fire naturally hurts growth
			...
			if (tileValue === Tile.AIRPORT) zoneSize = 5;else if (tileValue >= Tile.PORTBASE) zoneSize = 3;else if (tileValue < Tile.PORTBASE) zoneSize = 2; // Make remaining tiles of the zone bulldozable
		...
		static incRateOfGrowth(blockMaps, x, y, growthDelta) {
			let currentRate = blockMaps.rateOfGrowthMap.worldGet(x, y); // TODO why the scale of 4 here
	...
	const Messages = Object.defineProperties({}, messageData); //var Messages = Object.defineProperties({}, messageData);
	...
	Object.defineProperty(Messages, 'crashes', MiscUtils.mcd(crashes)); //export { Messages };
	...
	const Text = function () {
		...
		var zoneTypes = ['Clear', 'Water', 'Trees', 'Rubble', 'Flood', 'Radioactive Waste', 'Fire', 'Road', 'Power', 'Rail', 'Residential', 'Commercial', 'Industrial', 'Seaport', 'Airport', 'Coal Power', 'Fire Department', 'Police Department', 'Stadium', 'Nuclear Power', 'Draw Bridge', 'Radar Dish', 'Fountain', 'Industrial', 'Steelers 38	Bears 3', 'Draw Bridge', 'Ur 238']; // Evaluation window
		...
		problems[Micro.FIRE] = 'Fire'; // months

		var months = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec']; // Tool strings
	...
	class TrainSprite extends BaseSprite {
		constructor(map, spriteManager, x, y) {
			...
			this.TrainPic2 = [1, 2, 1, 2, 5]; // Frame values

			this.NORTHSOUTH = 1;
			this.EASTWEST = 2;
			this.NWSE = 3;
			this.NESW = 4;
			this.UNDERWATER = 5; // Direction values
		...
		move(spriteCycle, messageManager, disasterManager, blockMaps) {
			...
			this.y += this.yDelta[this.dir]; // Find a new direction.
	...
	class BoatSprite extends BaseSprite {
		...
		move(spriteCycle, messageManager, disasterManager, blockMaps) {
			...
			if (this.count === 0) {
				...
				this.count = 9; // If already executing a turn, continue to do so
				...
				for (dir = startDir; dir < startDir + 8; dir++) {
					...
					if (this.map.testBounds(x, y)) {
						tileValue = this.map.getTileValue(x, y); // Test for a suitable water tile
	...
	class MonsterSprite extends BaseSprite {
		...
		move(spriteCycle, messageManager, disasterManager, blockMaps) {
			if (this.soundCount > 0) this.soundCount--; // Frames 1 - 12 are diagonal sprites, 3 for each direction.
			...
			if (currentDir < 4) {
				...
				if (!(math.getRandom16() & 3)) {
					if (math.getRandom16() & 1) frame = this.diagonals1[frame];else frame = this.diagonals2[frame]; // We mung currentDir and frame here to
	...
	class AirplaneSprite extends BaseSprite {
		...
		move(spriteCycle, messageManager, disasterManager, blockMaps) {
			...
			if (disasterManager.enableDisasters) {
				...
				for (let i = 0; i < spriteList.length; i++) {
					let s = spriteList[i]; //if (s.frame === 0 || s === sprite) continue;
	...
	class TornadoSprite extends BaseSprite {
		...
		move(spriteCycle, messageManager, disasterManager, blockMaps) {
			let frame;
			frame = this.frame; // If middle frame, move right or left
			...
			for (let i = 0; i < spriteList.length; i++) {
				let s = spriteList[i]; // Explode vulnerable sprites
	...
	*/
	...
	class EvaluationUtils {
		static getTrafficAverage(blockMaps, census) {
			...
			var trafficAverage = Math.floor(trafficTotal / count) * 2.4; //census.trafficAverage = trafficAverage //??
		...
		static getUnemployment(census) {
			...
			if (b === 0) return 0; // Ratio total people / working. At least 1.
	...
	class Evaluation {
		...
		cityEvaluation(simData) {
			...
			if (census.totalPop > 0) {
				Micro.problemData = []; //var problemTable = [];
		...
		doProblems(census, budget, blockMaps) {
			...
			this.voteProblems(); // Rank the problems
		...
		getScore(simData) {
			...
			score = (250 - Math.min(score, 250)) * 4; //score = Math.min(score, 256);
			...
			if (valves.indCap) score = Math.round(score * demandPenalty); // Penalize if roads/rail underfunded

			if (budget.roadEffect < budget.MAX_ROAD_EFFECT) score -= budget.MAX_ROAD_EFFECT - budget.roadEffect; // Penalize player by up to 10% for underfunded police and fire services
			...
			score = Math.round(score * scale); // Penalize player for having fires and a burdensome tax rate
			...
			if (scale > 0.0) score = Math.round(score * (census.poweredZoneCount / scale)); // Force in to range 0-1000. New score is average of last score and new computed value
	...
	class Valves {
		...
		setValves(gameLevel, census, budget) {
			...
			var employment, labourBase; // Residential zones scale their population index when reporting it to the census

			var normalizedResPop = census.resPop / resPopDenom;
			census.totalPop = Math.round(normalizedResPop + census.comPop + census.indPop); // A lack of developed commercial and industrial zones means there are no employment opportunities, which constrain
			...
			if (census.resPop > 0) employment = (census.comHist10[1] + census.indHist10[1]) / normalizedResPop;else employment = 1; // Given the employment rate, calculate expected migration, add in births, and project the new population.

			var migration = normalizedResPop * (employment - 1);
			var births = normalizedResPop * birthRate;
			var projectedResPop = normalizedResPop + migration + births; // Examine how many zones require workers
			...
			labourBase = math.clamp(labourBase, 0.0, labourBaseMax); // Project future industry and commercial needs, taking into account available labour, and competition from
			// other global cities
			...
			projectedIndPop = Math.max(projectedIndPop, projectedIndPopMin); // Calculate the expected percentage changes in each population type
			...
			indRatio = Math.min(indRatio, indRatioMax); // Global tax and game level effects.
			...
			indRatio = (indRatio - 1) * taxTableScale + Micro.taxTable[z]; // Ratios are velocity changes to valves.
	...
	class Budget {
		constructor() {
			...
			this.cityTax = 7;
			this.cashFlow = 0;
			this.taxFund = 0; // These values denote how much money is required to fully maintain the relevant services

			this.roadMaintenanceBudget = 0;
			this.fireMaintenanceBudget = 0;
			this.policeMaintenanceBudget = 0; // Percentage of budget used

			this.roadPercent = 1;
			this.firePercent = 1;
			this.policePercent = 1; // Cash value of spending. Should equal Math.round(_Fund * _Percent)
		...
		_calculateBestPercentages() {
			...
			var total = this.roadSpend + this.fireSpend + this.policeSpend; // If we don't have any services on the map, we can bail early
			...
			var fireCost = 0;
			var policeCost = 0;
			var cashRemaining = this.totalFunds + this.taxFund; // Spending priorities: road, fire, police
		...
		doBudgetNow(fromWindow) {
			...
			var cashRemaining = this.totalFunds + this.taxFund - totalCost; // Autobudget
		...
		updateFundEffects() {
			...
			this.roadSpend = Math.round(this.roadMaintenanceBudget * this.roadPercent);
			this.fireSpend = Math.round(this.fireMaintenanceBudget * this.firePercent);
			this.policeSpend = Math.round(this.policeMaintenanceBudget * this.policePercent); // Update the effect this level of spending will have on infrastructure deterioration
		...
		collectTax(gameLevel, census) {
			this.cashFlow = 0; // How much would it cost to fully fund every service?
	...
	class Census {
		constructor() {
			...
			this.changed = false;
			this.crimeRamp = 0;
			this.pollutionRamp = 0; // Set externally
			...
			var createArray = function (arrName) {
				...
				this[arrName] = []; //for (var a = 0; a < 120; a++) this[arrName][a] = 0;
	...
	class Position {
		constructor(pos, deltaX, deltaY) {
			...
			if (arguments.length === 2 && !this.isNumber(pos) && !pos.isPosition) throw new Error('Position constructor called with bad existing position ' + pos + ' ' + deltaX); // This overloaded constructor accepts the following parameters
		...
		isNumber(v) {
			return !isNaN(v); //return typeof(v) === 'number';
		}
	...
	class PowerManager {
		...
		doPowerScan(census) {
			...
			this.powerGridMap.clear(); // Power that the combined coal and nuclear power plants can deliver.
		...
		coalPowerFound(map, x, y, simData) {
			...
			this._powerStack.push(new Position(x, y)); // Ensure animation runs


			var dX = [-1, 2, 1, 2];
			var dY = [-1, -1, 0, 0]; // Ensure animation bits set no animation for 3d
		...
		nuclearPowerFound(map, x, y, simData) {
			if (!simData) simData = Micro.simData;
			var meltdownTable = [30000, 20000, 10000]; // TODO With the auto repair system, zone gets repaired before meltdown
			...
			this._powerStack.push(new Position(x, y)); //console.log(x, y, new map.Position(x, y))
	...
	class Traffic {
		...
		addToTrafficDensityMap(blockMaps) {
			...
			while (this._stack.length > 0) {
				let pos = this._stack.pop(); // Could this happen?!?
				...
				if (tileValue >= Tile.ROADBASE && tileValue < Tile.POWERBASE) {
					...
					trafficDensityMap.worldSet(pos.x, pos.y, traffic); // Attract traffic copter to the traffic
		...
		tryGo(pos, dirLast) {
			let directions = []; // Find connections from current position.
	...
	class DisasterManager {
		constructor(map, spriteManager, gameLevel) {
			...
			this.Dx = [0, 1, 0, -1];
			this.Dy = [-1, 0, 1, 0]; // TODO enable disasters
			//Object.defineProperty(this, 'disastersEnabled', MiscUtils.mcd(false));
		}

		doDisasters(census) {
			if (this._floodCount) this._floodCount--; // TODO Scenarios
		...
		scenarioDisaster() {// TODO Scenarios
					...
					if (math.getChance(7)) {
						...
						if (this._map.testBounds(xx, yy)) {
							...
							if (tile.isCombustible() || tileValue === Tile.DIRT || tileValue >= Tile.WOODS5 && tileValue < Tile.FLOOD) {
								...
								this._map.setTile(xx, yy, Tile.FLOOD + math.getRandom(2), 0); //this._map.setTo(xx, yy, new Tiles(Tile.FLOOD + math.getRandom(2)));
		...
		doMeltdown(x, y) {
			...
			let i, dY, dX, tile; // Whole power plant is at fire
	...
	const Residential = {
		...
		buildHouse: function (map, x, y, lpValue) {
			let best = 0;
			let bestScore = 0; //	Deliberately ordered so that the centre tile is at index 0
			...
			if (best > 0 && map.testBounds(x + xDelta[best], y + yDelta[best])) map.setTile(x + xDelta[best], y + yDelta[best], Tile.HOUSE + math.getRandom(2) + lpValue * 3, Tile.BLBNCNBIT); //map.setTo(x + xDelta[best], y + yDelta[best], new Tiles(Tile.HOUSE + math.getRandom(2) + lpValue * 3, Tile.BLBNCNBIT));
		...
		growZone: function (map, x, y, blockMaps, population, lpValue, zonePower) {
			let pollution = blockMaps.pollutionDensityMap.worldGet(x, y); // Cough! Too polluted noone wants to move here!
		...
		degradeZone: function (map, x, y, blockMaps, population, lpValue, zonePower) {
			...
			if (population === 16) {
				...
				for (yy = y - 1; yy <= y + 1; yy++) {
					for (xx = x - 1; xx <= x + 1; xx++) {
						if (xx === x && yy === y) continue;
						map.setTile(x, y, Tile.LHTHR + lpValue + math.getRandom(2), Tile.BLBNCNBIT); //map.setTo(x, y, new Tiles(Tile.LHTHR + lpValue + math.getRandom(2), Tile.BLBNCNBIT));
					}
				}
			...
			for (xx = x - 1; xx <= x + 1; xx++) {
				for (yy = y - 1; yy <= y + 1; yy++) {
					...
					if (currentValue >= Tile.LHTHR && currentValue <= Tile.HHTHR) {
						...
						map.setTile(xx, yy, freeZone[i] + Tile.RESBASE, Tile.BLBNCNBIT); //map.setTo(xx, yy, new Tiles(freeZone[i] + Tile.RESBASE, Tile.BLBNCNBIT));
		...
		residentialFound: function (map, x, y, simData) {
			if (!simData) simData = Micro.simData; // If we choose to grow this zone, we will fill it with an index in the range 0-3 reflecting the land value and
			// pollution scores (higher is better). This is then used to select the variant to build

			let lpValue; // Notify the census

			simData.census.resZonePop += 1; // Also, notify the census of our population
			...
			let trafficOK = Micro.ROUTE_FOUND; // Occasionally check to see if the zone is connected to the road network. The chance of this happening increases
			...
			if (population > math.getRandom(35)) {
				...
				trafficOK = simData.traffic.makeTraffic(x, y, simData.blockMaps, ZoneUtils.isCommercial); // If we're not connected to the road network, then going shopping will be a pain. Move out.
			...
			if (tileValue === Tile.FREEZ || math.getChance(7)) {
				...
				let zoneScore = simData.valves.resValve + locationScore; // Naturally unpowered zones should be penalized

				if (!zonePower) zoneScore = -500; // The residential demand valve has range -2000 to 2000, so taking into account the "no traffic" and
		...
		makeHospital: function (map, x, y, simData, zonePower) {
			if (!simData) simData = Micro.simData; // We only build a hospital if the population requires it
		...
		hospitalFound: function (map, x, y, simData) {
			if (!simData) simData = Micro.simData;
			simData.census.hospitalPop += 1; // Degrade to an empty zone if a hospital is no longer sustainable
	...
	const Commercial = {
		...
		growZone: function (map, x, y, blockMaps, population, lpValue, zonePower) {
			...
			if (population > landValue) return; // This zone is desirable, and seemingly not to crowded. Switch to the next category of zone.
		...
		commercialFound: function (map, x, y, simData) {
			if (!simData) simData = Micro.simData; // lpValue will be filled if we actually decide to trigger growth/decay. It will be an index of the land/pollution
			// value in the range 0-3

			var lpValue; // Notify the census

			simData.census.comZonePop += 1; // Calculate the population level for this tile, and add to census

			var tileValue = map.getTileValue(x, y);
			var population = Commercial.getZonePopulation(map, x, y, tileValue);
			simData.census.comPop += population;
			var zonePower = map.getTile(x, y).isPowered(); // Occasionally check to see if the zone is connected to the transport network (the chance of this happening
			...
			if (population > math.getRandom(5)) {
				...
				trafficOK = simData.traffic.makeTraffic(x, y, simData.blockMaps, ZoneUtils.isIndustrial); // Trigger outward migration if not connected to road network
			...
			if (math.getChance(7)) {
				...
				var zoneScore = simData.valves.comValve + locationScore; // Unpowered zones should of course be penalized

				if (!zonePower) zoneScore = -500; // The commercial demand valve has range -1500 to 1500, so taking into account the "no traffic" and
	...
	const Industrial = {
		...
		setAnimation: function (map, x, y, tileValue, isPowered) {
			if (tileValue < Tile.IZB) return; // There are only 7 different types of populated industrial zones.
		...
		industrialFound: function (map, x, y, simData) {
			if (!simData) simData = Micro.simData;
			simData.census.indZonePop += 1; // Calculate the population level for this tile, and add to census

			var tileValue = map.getTileValue(x, y);
			var population = Industrial.getZonePopulation(map, x, y, tileValue);
			simData.census.indPop += population; // Set animation bit if appropriate

			var zonePower = map.getTile(x, y).isPowered();
			if (!simData.is3D) Industrial.setAnimation(map, x, y, tileValue, zonePower); // Occasionally check to see if the zone is connected to the transport network (the chance of this happening
			...
			if (population > math.getRandom(5)) {
				...
				trafficOK = simData.traffic.makeTraffic(x, y, simData.blockMaps, ZoneUtils.isResidential); // Trigger outward migration if not connected to road network (unless the zone is already empty)
			...
			if (math.getChance(7)) {
				var zoneScore = simData.valves.indValve + (trafficOK === Micro.NO_ROAD_FOUND ? -1000 : 0); // Unpowered zones should of course be penalized

				if (!zonePower) zoneScore = -500; // The industrial demand valve has range -1500 to 1500, so taking into account the "no traffic" and
	...
	const Road = {
		...
		roadFound: function (map, x, y, simData) {
			...
			if (!currentTile.isCombustible()) {
				...
				simData.census.roadTotal += 4; //if ( Road.doBridge(map, x, y, tileValue, simData)) return;
			...
			let currentDensity = simData.blockMaps.trafficDensityMap.worldGet(x, y) >> 6; // Force currentDensity in range 0-3 (trafficDensityMap values are capped at 240)

			if (currentDensity > 1) currentDensity -= 1;
			if (currentDensity === density) return;
			let newValue = (tileValue - Tile.ROADBASE & 15) + densityTable[currentDensity]; // Preserve all bits except animation
	...
	const Transport = {
		...
		railFound: function (map, x, y, simData) {
			...
			if (simData.budget.shouldDegradeRoad()) {
				if (math.getChance(511)) {
					let currentTile = map.getTile(x, y); // Don't degrade tiles with power lines

					if (currentTile.isConductive()) return;

					if (simData.budget.roadEffect < (math.getRandom16() & 31)) {
						let mapValue = currentTile.getValue(); // Replace bridge tiles with water, otherwise rubble
	...
	const handleService = function (censusStat, budgetEffect, blockMap) {
		return function (map, x, y, simData) {
			...
			var isPowered = map.getTile(x, y).isPowered(); // Unpowered buildings are half as effective
	...
	*/
	...
	const MiscTiles = {
		registerHandlers: function (mapScanner, repairManager) {
			...
			mapScanner.addAction(ZoneUtils.isFlood, MiscTiles.floodFound, true); //mapScanner.addAction(ZoneUtils.isManualExplosion, MiscTiles.explosionFound, true);
		},
		fireFound: function (map, x, y, simData) {
			if (!simData) simData = Micro.simData;
			simData.census.firePop += 1;
			if ((math.getRandom16() & 3) !== 0) return;
			let i, xTem, yTem, tile; // Try to set neighbouring tiles on fire as well
				...
				if (math.getChance(7)) {
					...
					if (map.testBounds(xTem, yTem)) {
						...
						if (tile.isZone()) {
							...
							ZoneUtils.fireZone(map, x, y, simData.blockMaps); // Industrial zones etc really go boom
			...
			if (i > 100) rate = 1;else if (i > 20) rate = 2;else if (i > 0) rate = 3; // Decide whether to put out the fire.
	...
	class MapUtils {
		...
		static getPollutionValue(tileValue) {
			if (tileValue < Tile.POWERBASE) {
				...
				if (tileValue >= Tile.HTRFBASE) return 75; // Low traffic not so much

				if (tileValue >= Tile.LTRFBASE) return 50;

				if (tileValue < Tile.ROADBASE) {
					// Fire = carbon monoxide = a bad score for you
					if (tileValue > Tile.FIREBASE) return 90; // Radiation. Top of the charts.
			...
			if (tileValue <= Tile.LASTIND) return 0; // Industrial zones, however...

			if (tileValue < Tile.PORTBASE) return 50; // Coal power plants are bad
		...
		static pollutionTerrainLandValueScan(map, census, blockMaps) {
			...
			let tempMap2 = blockMaps.tempMap2; // tempMap3 will be used to record raw terrain information, i.e. if the the land is developed. This will be
			...
			while (x--) {
				...
				while (y--) {
					pollutionLevel = 0;
					developed = false; // The land value map has a chunk size of 2
					...
					if (developed) {
						...
						landValue = landValue << 2; // Land in the same neighbourhood as unspoiled land is more valuable...

						landValue += terrainDensityMap.get(x >> 1, y >> 1); // ... and polluted land obviously is less valuable

						landValue -= pollutionDensityMap.get(x, y); // ... getting mugged won't help either
			...
			if (developedTileCount > 0) census.landValueAverage = Math.floor(totalLandValue / developedTileCount);else census.landValueAverage = 0; // Smooth the pollution map twice
			...
			let pollution; // We iterate over the now-smoothed pollution map rather than using the block map's copy routines
			...
			for (x = 0; x < map.width; x += pollutionDensityMap.blockSize) {
				for (y = 0; y < map.height; y += pollutionDensityMap.blockSize) {
					...
					if (pollution !== 0) {
						pollutedTileCount++;
						totalPollution += pollution; // Note the most polluted location: any monsters will be drawn there (randomly choosing one
		...
		static crimeScan(census, blockMaps) {
			...
			let x,
					...
					value; // Scan the map, looking for developed land, as it can attract crime.
			...
			for (x = 0; x < width; x += blockSize) {
				for (y = 0; y < height; y += blockSize) {
					...
					if (value > 0) {
						crimeZoneCount += 1; // Force value in the range -122 to 128. Lower valued pieces of land attract more crime.

						value = 128 - value; // Add population density (a value between 0 and 510). value now lies in range -260 - 382.
						// Denser areas attract more crime.

						value += populationDensityMap.worldGet(x, y); // Clamp in range -260 to 300

						value = Math.min(value, 300); // If the police are nearby, there's no point committing the crime of the century

						value -= policeStationMap.worldGet(x, y); // Force in to range 0-250
		...
		static fillCityCentreDistScoreMap(map, blockMaps) {
			...
			while (x--) {
				...
				while (y--) {
					...
					value = Math.floor(MapUtils.getCityCentreDistance(map, x * 8, y * 8) / 2); // Now, we scale up by a factor of 4. We're in the range 0 - 128

					value = value * 4; // And finally, subtract from 64, leaving us a score in the range -64 to 64
		...
		static populationDensityScan(map, blockMaps) {
			...
			blockMaps.populationDensityMap; // We will sum all the coordinates that contain zones into xTot and yTot. They are used in our city centre heuristic.
			...
			while (x--) {
				...
				while (y--) {
					...
					if (tile.isZone()) {
						tileValue = tile.getValue(); // Ask the zone to calculate its population, scale it up, then clamp in the range 0-254

						population = MapUtils.getPopulationDensity(map, x, y, tileValue) * 8;
						population = Math.min(population, 254); // The block size of population density is 2x2, so there can only be 1 zone per block
			...
			MapUtils.fillCityCentreDistScoreMap(map, blockMaps); // Compute new city center
	...
	class Simulation {
		constructor(gameMap, gameLevel, speed, is3D, savedGame) {
			if (gameLevel !== Micro.LEVEL_EASY && gameLevel !== Micro.LEVEL_MED && gameLevel !== Micro.LEVEL_HARD) throw new Error('Invalid level!'); // if (speed !== Micro.SPEED_PAUSED && speed !== Micro.SPEED_SLOW && speed !== Micro.SPEED_MED && speed !== Micro.SPEED_FAST) throw new Error('Invalid speed!');
			...
			this.startingYear = 1900; // Last valves updated to the user

			this.resValveLast = 0;
			this.comValveLast = 0;
			this.indValveLast = 0; // Last date sent to front end

			this._cityYearLast = -1;
			this._cityMonthLast = -1; // Last time we relayed a message from PowerManager to the front-end

			this._lastPowerMessage = null;
			this.infos = []; // And now, the main cast of characters
			...
			if (savedGame) {
				this.load(savedGame); //this.cityPopLast = savedGame.totalPop;			
			} else {
		...
		load(saveData) {
			...
			for (let i = 0, l = Micro.savePropsVar.length; i < l; i++) this[Micro.savePropsVar[i]] = saveData[Micro.savePropsVar[i]]; //this.map.load(saveData);
		...
		updateInfo() {
			...
			this.infos[7] = this.valves.indValve; //this.infos[8] = '' // message
		...
		simFrame() {
			...
			if (this.speed === Micro.SPEED_PAUSED) return false; // Default to slow speed
		...
		init() {
			this.prevTime = -1; // Register actions
			...
			this.clearCensus(); //this.mapScanner.mapScan(0, this.map.width, simData);
			...
			MapUtils.fireAnalysis(this.blockMaps); //this.census.totalPop = 1;
	...
	class AnimationManager {
		constructor(map, animationPeriod, blinkPeriod) {
			...
			this.count = 1; // When painting we keep track of what frames
	...
	class GameMap {
		constructor(width = 128, height = 128, defaultValue) {
			this.isIsland = false;
			this.pp = []; //this.Position = new PositionMaker( width, height ); // !! find better way
		...
		upLayer(id, value, x, y) {
			...
			this.tilesData[id] = value; //if( this.goodValue(value) ){
		...
		load(saveData) {
			let x,
					...
					i = 0,
					map = saveData.map,
					tiles = saveData.tileValue; // GAME PROPS
		...
		getTile(x, y, newTile) {
			...
			let tile = this.data[tileIndex]; //var tileIndex = this._calculateIndex(x, y);
			// Return the original tile if we're not given a tile to fill

			if (!newTile) return tile;
			newTile.set(tile);
			return tile; //if (!(tileIndex in this.data)) this.data[tileIndex] = new Tiles(this.defaultValue);
			//return this.data[tileIndex];
		}

		getTileValue(x, y) {
			let e = new Error('Invalid parameter');
			if (arguments.length < 1) throw e; // Argument-shuffling
		...
		getTiles(x, y, w, h) {
			let e = new Error('Invalid parameter');
			if (arguments.length < 3) throw e; // Argument-shuffling
			...
			for (let a = y, ylim = y + h; a < ylim; a++) {
				...
				for (let b = x, xlim = x + w; b < xlim; b++) {
					let tileIndex = this.getId(b, a); //if (!(tileIndex in this.data)) this.data[tileIndex] = new Tiles(this.defaultValue);
		...
		getTileValues(x, y, w, h, result) {
			...
			let width = this.width;
			let height = this.height; // Result is stored in row-major order
			...
			for (let a = y, ylim = y + h; a < ylim; a++) {
				for (let b = x, xlim = x + w; b < xlim; b++) {
					...
					let tileIndex = b + a * width; //result[(a - y) * w + (b - x)] = this._data[tileIndex].getRawValue();
		...
		setTo(x, y, tile) {
			...
			let isNew = this.testOld(id, value);
			this.data[id] = tile; // this.data[ id ].setValue(value);
		...
		setTileValue(x, y, value) {
			...
			let id = this.getId(x, y);
			let isNew = this.testOld(id, value); //if (!(tileIndex in this.data)) this.data[tileIndex] = new Tiles(this.defaultValue);
		...
		setTileFlags(x, y, flags) {
			...
			if (arguments.length < 2) throw e; // Argument-shuffling
		...
		removeTileFlags(x, y, flags) {
			...
			let id = this.getId(x, y);
			this.data[id].removeFlags(flags); ///this.upLayer( id, this.data[id].getValue(), x, y )
		}
	...
	class MapGenerator {
		...
		construct(w, h, debug = false) {
			...
			this.map = new GameMap(w || Micro.MAP_WIDTH, h || Micro.MAP_HEIGHT); //this.map.makePP()
			...
			if (Micro.TERRAIN_CREATE_ISLAND === 1) this.makeNakedIsland();else this.clearMap(); // Lay a river.
			...
			this.smoothRiver();
			this.cleanBorder(); // And add trees.
		...
		treeSplash(x, y) {
			...
			while (numTrees > 0) {
				...
				treePos.move(dir); // XXX Should use the fact that positions return success/failure for moves
		...
		smoothRiver() {
			...
			map.pp.forEach(v => {
				...
				if (map.getTileValue(x, y) === Tile.REDGE) {
					...
					if (temp !== Tile.RIVER && math.getRandom(1)) temp++; //map.setTileValue(x, y, temp, 0);
			...
			map.pp.forEach(v => {
				...
				if (tile >= Tile.WATER_LOW && tile <= Tile.WATER_HIGH) {
					...
					for (dir = Direction.BEGIN; dir < Direction.END; dir = Direction.increment90(dir)) {
						tile = map.getTileFromMap(pos, dir, Tile.WATER_LOW); // If nearest object is not water:
	...
	class BuildingTool extends BaseToolConnector {
		...
		prepareBuildingSite(leftX, topY) {
			...
			let posX, posY, tileValue; // Check whether the tiles are clear
	...
	class BulldozerTool extends BaseToolConnector {
		...
		doTool(x, y, blockMaps, messageManager) {
			...
			if (zoneSize > 0) {
				this.addCost(this.bulldozerCost); //this._map.powerData[ this._map.getId( x, y ) ] = 0;
	...
	class QueryTool extends BaseTool {
		...
		classifyPopulationDensity(x, y, blockMaps) {
			var density = blockMaps.populationDensityMap.worldGet(x, y); //if (debug) document.getElementById("queryDensityRaw").innerHTML=density;

			density = density >> 6;
			density = density & 3;
			this.txt += 'Density: ' + TXT.densityStrings[density] + '<br>'; //document.getElementById("queryDensity").innerHTML=TXT.densityStrings[density];
		}

		classifyLandValue(x, y, blockMaps) {
			var landValue = blockMaps.landValueMap.worldGet(x, y); //if (debug) document.getElementById("queryLandValueRaw").innerHTML=landValue;

			var i = 0;
			if (landValue >= 150) i = 3;else if (landValue >= 80) i = 2;else if (landValue >= 30) i = 1; //var text = TXT.landValueStrings[i];

			this.txt += 'Value: ' + TXT.landValueStrings[i] + '<br>'; //document.getElementById("queryLandValue").innerHTML=text;
		}

		classifyCrime(x, y, blockMaps) {
			var crime = blockMaps.crimeRateMap.worldGet(x, y); //if (debug) document.getElementById("queryCrimeRaw").innerHTML=crime;

			crime = crime >> 6;
			crime = crime & 3;
			this.txt += 'Crime: ' + TXT.crimeStrings[crime] + '<br>'; //document.getElementById("queryCrime").innerHTML=TXT.crimeStrings[crime];
		}

		classifyPollution(x, y, blockMaps) {
			var pollution = blockMaps.pollutionDensityMap.worldGet(x, y); //if (debug) document.getElementById("queryPollutionRaw").innerHTML=pollution;

			pollution = pollution >> 6;
			pollution = pollution & 3;
			this.txt += 'Pollution: ' + TXT.pollutionStrings[pollution] + '<br>'; //document.getElementById("queryPollution").innerHTML=TXT.pollutionStrings[pollution];
		}

		classifyRateOfGrowth(x, y, blockMaps) {
			var rate = blockMaps.rateOfGrowthMap.worldGet(x, y); //if (debug) document.getElementById("queryRateRaw").innerHTML=rate;

			rate = rate >> 6;
			rate = rate & 3;
			this.txt += 'Growth: ' + TXT.rateStrings[rate]; //document.getElementById("queryRate").innerHTML=TXT.rateStrings[rate];
		}
		...
		classifyZone(x, y) {
			...
			this.txt = 'Zone: ' + TXT.zoneTypes[index] + '<br>'; //document.getElementById("queryZoneType").innerHTML=TXT.zoneTypes[index];
		}
	...
	var Game;
	var isWorker = true;
	var returnMessage; //var ab = new ArrayBuffer( 1 );
	//CityGame.post( ab, [ab] );
	...
	class CityGame {
		static message(e) {
			...
			if (p == "MAPCLICK") Game.mapClick(e.data.x, e.data.y, e.data.single || false); //if( p == "DESTROY" ) Game.destroy(e.data.x, e.data.y);
	...
	class MainGame {
		constructor(timestep) {
			...
			this.animsData = null; //this.tilesData = null;
		...
		playMap(loading) {
			...
			if (loading) {
				money = this.savedGame.totalFunds; //this.infos[3] = this.savedGame.totalPop;

				this.speed = this.savedGame.speed;
				this.difficulty = this.savedGame.difficulty;
				this.simulation = new Simulation(this.map, this.difficulty, this.speed, true, this.savedGame); //this.processMessages(Messages.EVAL_UPDATED);
			...
			this.simulation.budget.setFunds(money); //messageMgr.sendMessage(Messages.FUNDS_CHANGED, money);

			this.processMessages(messageMgr.getMessages()); // update simulation time

			this.isPaused = false; //if(!trans) 
		...
		calculateSprites() {
			...
			var i = this.sprites.length; //this.spritesData = new M_ARRAY_TYPE(i);
		...
		destroy(x, y) {
			console.log('isDestroy'); //console.log( 'destuct ', x, y )
			//this.mapClick(x,y);
		...
		makeLoadGame(gameData, atStart) {
			...
			clearInterval(this.timer);
			this.savedGame = JSON.parse(gameData); //this.simulation.load(this.savedGame);
