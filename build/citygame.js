...
(function (global, factory) {
	...
	*/
	...
	class Budget {
		...
		get roadFund() {
			return this.roadMaintenanceBudget;
		}
		get fireFund() {
			return this.fireMaintenanceBudget;
		}
		get policeFund() {
	...
	class Census {
		...
		take10Census(budget) {
			...
			if (this.hospitalPop < resPopScaled) this.needHospital = 1;else if (this.hospitalPop > resPopScaled) this.needHospital = -1;else this.needHospital = 0;
			this.changed = true;
		}
		take120Census() {
	...
	class DisasterManager {
		...
		makeEarthquake() {
			...
			EventEmitter.emitEvent(Messages.EARTHQUAKE, {
				showable: true,
				x: this._map.cityCentreX,
				y: this._map.cityCentreY
			});
			let i, x, y;
	...
	*/
	...
	class MapUtils {
		...
		static crimeScan(census, blockMaps) {
			...
			let x,
				y,
				width = crimeRateMap.gameMapWidth,
				height = crimeRateMap.gameMapHeight,
				value;
	...
	const ACHIEVEMENTS = [{
		id: 'first_zone',
		name: 'Founder',
		desc: 'Place your first zone',
		check: s => s.census.totalPop > 0
	}, {
		id: 'pop_500',
		name: 'Small Settlement',
		desc: 'Reach 500 population',
		check: s => s.evaluation.cityPop >= 500
	}, {
		id: 'pop_2000',
		name: 'Growing Town',
		desc: 'Reach 2,000 population',
		check: s => s.evaluation.cityPop >= 2000
	}, {
		id: 'pop_10000',
		name: 'City Founder',
		desc: 'Reach 10,000 population',
		check: s => s.evaluation.cityPop >= 10000
	}, {
		id: 'pop_50000',
		name: 'Capital Builder',
		desc: 'Reach 50,000 population',
		check: s => s.evaluation.cityPop >= 50000
	}, {
		id: 'pop_100000',
		name: 'Metropolis Master',
		desc: 'Reach 100,000 population',
		check: s => s.evaluation.cityPop >= 100000
	}, {
		id: 'pop_500000',
		name: 'Megalopolis',
		desc: 'Reach 500,000 population',
		check: s => s.evaluation.cityPop >= 500000
	}, {
		id: 'rich_100k',
		name: 'Prosperous',
		desc: 'Accumulate $100,000',
		check: s => s.budget.totalFunds >= 100000
	}, {
		id: 'rich_1m',
		name: 'Tycoon',
		desc: 'Accumulate $1,000,000',
		check: s => s.budget.totalFunds >= 1000000
	}, {
		id: 'low_crime',
		name: 'Safe Streets',
		desc: 'Keep crime average below 10',
		check: s => s.census.crimeAverage < 10 && s.census.totalPop > 100
	}, {
		id: 'no_pollution',
		name: 'Green City',
		desc: 'Keep pollution average below 15',
		check: s => s.census.pollutionAverage < 15 && s.census.totalPop > 100
	}, {
		id: 'high_approval',
		name: 'Beloved Mayor',
		desc: 'Get 90%+ approval rating',
		check: s => s.evaluation.cityYes >= 90
	}, {
		id: 'high_score',
		name: 'Perfect City',
		desc: 'Achieve city score of 900+',
		check: s => s.evaluation.cityScore >= 900
	}, {
		id: 'nuclear_power',
		name: 'Nuclear Age',
		desc: 'Build a nuclear power plant',
		check: s => s.census.nuclearPowerPop > 0
	}, {
		id: 'airport_built',
		name: 'Sky\'s the Limit',
		desc: 'Build an airport',
		check: s => s.census.airportPop > 0
	}, {
		id: 'seaport_built',
		name: 'Harbor Master',
		desc: 'Build a seaport',
		check: s => s.census.seaportPop > 0
	}, {
		id: 'stadium_built',
		name: 'Sports Fan',
		desc: 'Build a stadium',
		check: s => s.census.stadiumPop > 0
	}, {
		id: 'full_services',
		name: 'Full Coverage',
		desc: 'Have police, fire, and hospital',
		check: s => s.census.policeStationPop > 0 && s.census.fireStationPop > 0 && s.census.hospitalPop > 0
	}, {
		id: 'survive_disaster',
		name: 'Resilient',
		desc: 'Survive a disaster',
		check: null
	},
	// Triggered manually
	{
		id: 'year_2000',
		name: 'Millennium',
		desc: 'Reach the year 2000',
		check: s => {
			let yr = Math.floor(s.cityTime / 48) + s.startingYear;
			return yr >= 2000;
		}
	}, {
		id: 'high_education',
		name: 'Educated City',
		desc: 'Reach education level 150+',
		check: s => s.census.educationLevel >= 150
	}, {
		id: 'high_health',
		name: 'Healthy City',
		desc: 'Reach health level 150+',
		check: s => s.census.healthLevel >= 150
	}, {
		id: 'happy_city',
		name: 'Utopia',
		desc: 'Reach happiness level 85+',
		check: s => s.census.happinessLevel >= 85
	}, {
		id: 'debt_free',
		name: 'Debt Free',
		desc: 'Pay off all municipal bond debt',
		check: s => s.budget.bondDebt === 0 && s.census.totalPop > 500
	}, {
		id: 'no_fire',
		name: 'Fireproof',
		desc: 'Keep fire severity at zero for a full evaluation cycle',
		check: s => s.census.firePop === 0 && s.census.fireStationPop > 0 && s.census.totalPop > 1000
	}, {
		id: 'park_builder',
		name: 'Park Builder',
		desc: 'Place 20 or more park tiles',
		check: s => (s.census.parkCount || 0) >= 20
	}];
	class Achievements {
		constructor() {
			this.unlocked = {};
	...
	class SeasonManager {
		...
		update(cityMonth) {
			let newSeason;
			if (cityMonth >= 2 && cityMonth <= 4) newSeason = Micro.SEASON_SPRING;else if (cityMonth >= 5 && cityMonth <= 7) newSeason = Micro.SEASON_SUMMER;else if (cityMonth >= 8 && cityMonth <= 10) newSeason = Micro.SEASON_AUTUMN;else newSeason = Micro.SEASON_WINTER;
			...
			switch (this.cityTime & 63) {
				...
				case 30:
					if (this.census.comPop > 100 && this.census.airportPop === 0) {
						this.messageManager.sendMessage(Messages.NEED_AIRPORT);
				...
				case 45:
					...
					break;
				case 47:
					if (this.census.needHospital > 0) this.messageManager.sendMessage(Messages.NEED_HOSPITAL);
					break;
				case 48:
					if (this.census.totalPop > 60 && this.census.policeStationPop === 0) this.messageManager.sendMessage(Messages.NEED_POLICE_STATION);
					break;
				case 50:
					if (this.census.totalPop > 200 && this.census.educationLevel < 30) this.messageManager.sendMessage(Messages.NEED_SCHOOLS);
					break;
				case 51:
					if (this.budget.cityTax > 12) this.messageManager.sendMessage(Messages.TAX_TOO_HIGH);
					break;
				case 52:
					if (this.budget.bondDebt > this.budget.MAX_BOND_DEBT * 0.8) this.messageManager.sendMessage(Messages.BOND_HIGH_DEBT);
					break;
				case 54:
					if (this.budget.roadEffect < Math.floor(5 * Micro.MAX_ROAD_EFFECT / 8) && this.census.roadTotal > 30) this.messageManager.sendMessage(Messages.ROAD_NEEDS_FUNDING);
					break;
				case 57:
					if (this.budget.fireEffect < Math.floor(7 * Micro.MAX_FIRESTATION_EFFECT / 10) && this.census.totalPop > 20) this.messageManager.sendMessage(Messages.FIRE_STATION_NEEDS_FUNDING);
					break;
				case 60:
					if (this.budget.policeEffect < Math.floor(7 * Micro.MAX_POLICESTATION_EFFECT / 10) && this.census.totalPop > 20) this.messageManager.sendMessage(Messages.POLICE_NEEDS_FUNDING);
					break;
				case 63:
					if (EvaluationUtils.getTrafficAverage(this.blockMaps, this.census) > 60) this.messageManager.sendMessage(Messages.TRAFFIC_JAMS, -1, -1, true);
					break;
			}
		}
		checkGrowth() {
	...
	*/
	...
	class MainGame {
		...
		setDisaster(disaster) {
			...
			switch (disaster) {
				...
				case Micro.DISASTER_TORNADO:
					...
					break;
				case Micro.DISASTER_EARTHQUAKE:
					this.simulation.disasterManager.makeEarthquake();
					break;
			}
		...
		getEvaluation() {
			...
			pollutionAvg,
			// 3
			Math.round(trafficAvg),
			// 4
			census.educationLevel,
			// 5
			census.healthLevel,
			// 6
			census.happinessLevel,
			// 7
			unemployment,
			// 8
			season,
		...
		issueBond(amount) {
			...
			if (issued) {
				...
				this.simulation.cityHistory.addEvent('economic', 'Issued municipal bond of $' + amount + ' (total debt: $' + this.simulation.budget.bondDebt + ')', this.simulation.cityTime, this.simulation.startingYear);
			}
