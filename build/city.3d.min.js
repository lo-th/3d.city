...
Micro.pollutionTerrainLandValueScan=function(a,b,c){var d=c.tempMap1,e=c.tempMap3,f=c.landValueMap,g=c.terrainDensityMap,h=c.pollutionDensityMap,k=c.crimeRateMap,l,m;e.clear();var n=0,p=0;for(l=0;l<f.width;l++)for(m=0;m<f.height;m++){for(var q=0,w=!1,r=2*l,s=2*m,u=r;u<=r+1;u++)for(var t=s;t<=s+1;t++){var v=a.getTileValue(u,t);v>Tile.DIRT&&(v<Tile.RUBBLE?(v=e.get(l>>1,m>>1),e.set(l>>1,m>>1,v+15)):(q+=Micro.getPollutionValue(v),v>=Tile.ROADBASE&&(w=!0)))}q=Math.min(q,255);d.set(l,m,q);w?(q=34-Math.floor(Micro.getCityCentreDistance(a,
r,s)/2),q<<=2,q+=g.get(l>>1,m>>1),q-=h.get(l,m),190<k.get(l,m)&&(q-=20),q=Micro.clamp(q,1,250),f.set(l,m,q),n+=q,p++):f.set(l,m,0)}b.landValueAverage=0<p?Math.floor(n/p):0;Micro.smoothTemp1ToTemp2(c);Micro.smoothTemp2ToTemp1(c);for(l=g=f=e=0;l<h.mapWidth;l+=h.blockSize)for(m=0;m<h.mapHeight;m+=h.blockSize)if(k=d.worldGet(l,m),h.worldSet(l,m,k),0!==k&&(f++,g+=k,k>e||k===e&&Random.getChance(3)))e=k,a.pollutionMaxX=l,a.pollutionMaxY=m;b.pollutionAverage=f?Math.floor(g/f):0;Micro.smoothTerrain(c)};
...
Micro.crimeScan=function(a,b){var c=b.policeStationMap,d=b.policeStationEffectMap,e=b.crimeRateMap,f=b.landValueMap,g=b.populationDensityMap;Micro.smoothStationMap(c);Micro.smoothStationMap(c);Micro.smoothStationMap(c);for(var h=0,k=0,l=0;l<e.mapWidth;l+=e.blockSize)for(var m=0;m<e.mapHeight;m+=e.blockSize){var n=f.worldGet(l,m);0<n?(++k,n=128-n,n+=g.worldGet(l,m),n=Math.min(n,300),n-=c.worldGet(l,m),n-=d.worldGet(l,m),n=Micro.clamp(n,0,250),e.worldSet(l,m,n),h+=n):e.worldSet(l,m,0)}a.crimeAverage=
0<k?Math.floor(h/k):0;b.policeStationEffectMap=new Micro.BlockMap(c)};Micro.computeComRateMap=function(a,b){for(var c=b.comRateMap,d=0;d<c.width;d++)for(var e=0;e<c.height;e++){var f=Math.floor(Micro.getCityCentreDistance(a,8*d,8*e)/2),f=4*f,f=64-f;c.set(d,e,f)}};Micro.getPopulationDensity=function(a,b,c,d){return d<Tile.COMBASE?Residential.getZonePopulation(a,b,c,d):d<Tile.INDBASE?8*Commercial.getZonePopulation(a,b,c,d):d<Tile.PORTBASE?8*Industrial.getZonePopulation(a,b,c,d):0};
Micro.populationDensityScan=function(a,b){var c=b.tempMap1,d=b.tempMap2;c.clear();for(var e=0,f=0,g=0,h=0;h<a.width;h++)for(var k=0;k<a.height;k++){var l=a.getTile(h,k);l.isZone()&&(l=l.getValue(),l=8*Micro.getPopulationDensity(a,h,k,l),l=Math.min(l,254),c.worldSet(h,k,l),e+=h,f+=k,g++)}Micro.smoothTemp1ToTemp2(b);Micro.smoothTemp2ToTemp1(b);Micro.smoothTemp1ToTemp2(b);b.populationDensityMap=new Micro.BlockMap(d,function(a){return 2*a});Micro.computeComRateMap(a,b);0<g?(a.cityCentreX=Math.floor(e/
