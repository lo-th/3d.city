...
e,0.02,c+e),new THREE.Vector3(c+e,0.02,c+e),new THREE.Vector3(c+e,0.02,-c+e)];a.vertices.push(c[0],c[1],c[1],c[2],c[2],c[3],c[3],c[0]);b=new THREE.LineSegments(a,new THREE.LineBasicMaterial({color:b,linewidth:3,depthWrite:!1,depthTest:!1}));b.overdraw=!0;return b},build:function(a,b){if("query"!==this.currentTool.tool&&16!==this.currentTool.id)if(this.currentTool.build){var c=this.currentTool.size,e=0;this.isWithHeight&&(e=this.heightData[this.findHeightId(a,b)]);var f;1==c?f=[[a,b]]:3==c?f=[[a,b],
[a-1,b],[a+1,b],[a,b-1],[a-1,b-1],[a+1,b-1],[a,b+1],[a-1,b+1],[a+1,b+1]]:4==c?f=[[a,b],[a-1,b],[a+1,b],[a,b-1],[a-1,b-1],[a+1,b-1],[a,b+1],[a-1,b+1],[a+1,b+1],[a+2,b-1],[a+2,b],[a+2,b+1],[a+2,b+2],[a-1,b+2],[a,b+2],[a+1,b+2]]:6==c&&(f=[[a,b],[a-1,b],[a+1,b],[a,b-1],[a-1,b-1],[a+1,b-1],[a,b+1],[a-1,b+1],[a+1,b+1],[a+2,b-1],[a+2,b],[a+2,b+1],[a+2,b+2],[a-1,b+2],[a,b+2],[a+1,b+2],[a+3,b-1],[a+4,b-1],[a+3,b],[a+4,b],[a+3,b+1],[a+4,b+1],[a+3,b+2],[a+4,b+2],[a+3,b+3],[a+4,b+3],[a+3,b+4],[a+4,b+4],[a-1,
...
dragCenterposition:function(){if(0!=this.ease.x||0!=this.ease.z)this.easeRot.y=this.cam.horizontal*this.ToRad,this.unwrapDegrees(Math.round(this.cam.horizontal)),this.easeRot.x=Math.sin(this.easeRot.y)*this.ease.x+Math.cos(this.easeRot.y)*this.ease.z,this.easeRot.z=Math.cos(this.easeRot.y)*this.ease.x-Math.sin(this.easeRot.y)*this.ease.z,this.center.x+=this.easeRot.x,this.center.z-=this.easeRot.z,0>this.center.x&&(this.center.x=0),128<this.center.x&&(this.center.x=128),0>this.center.z&&(this.center.z=
0),128<this.center.z&&(this.center.z=128),this.moveCamera()},onMouseDown:function(a){a.preventDefault();var b,c;a.touches?(b=a.clientX||a.touches[0].pageX,c=a.clientY||a.touches[0].pageY):(b=a.clientX,c=a.clientY,this.mouse.button=a.which);this.mouse.ox=b;this.mouse.oy=c;this.rayVector.x=b/this.vsize.x*2-1;this.rayVector.y=2*-(c/this.vsize.y)+1;this.mouse.h=this.cam.horizontal;this.mouse.v=this.cam.vertical;this.mouse.down=!0;this.currentTool&&2>this.mouse.button&&(this.mouse.click=!0,this.currentTool.drag&&
(this.mouse.drag=!0))},onMouseUp:function(a){a.preventDefault();this.mouse.button=0;this.mouse.down=!1;this.mouse.drag=!1;null==this.currentTool&&(this.mouse.move=!0);this.ease.x=0;this.ease.z=0;document.body.style.cursor="auto"},onMouseMove:function(a){a.preventDefault();var b;a.touches?(b=a.clientX||a.touches[0].pageX,a=a.clientY||a.touches[0].pageY):(b=a.clientX,a=a.clientY);if(this.mouse.down){if(this.mouse.move||2===this.mouse.button)this.mouse.dragView=!1,document.body.style.cursor="crosshair",
this.cam.horizontal=0.3*(b-this.mouse.ox)+this.mouse.h,this.cam.vertical=0.3*-(a-this.mouse.oy)+this.mouse.v,this.moveCamera();if(this.mouse.dragView||3===this.mouse.button)document.body.style.cursor="move",this.mouse.move=!1,this.ease.x=(b-this.mouse.ox)/1E3,this.ease.z=(a-this.mouse.oy)/1E3}null!==this.currentTool&&(this.rayVector.x=b/this.vsize.x*2-1,this.rayVector.y=2*-(a/this.vsize.y)+1,this.rayTest())},onMouseWheel:function(a){a.preventDefault();var b=0;a.wheelDelta?b=-1*a.wheelDelta:a.detail&&
(b=20*a.detail);this.cam.distance+=b/80;1>this.cam.distance&&(this.cam.distance=1);150<this.cam.distance&&(this.cam.distance=150);this.moveCamera()},cleanGround:function(a){for(var b=a.length,c,e,f,h,g;b--;)e=a[b][0],f=a[b][1],h=Math.floor(e/16),g=Math.floor(f/16),c=h+8*g,this.miniCtx[c].drawImage(this.imageSrc,0,0,16*this.mu,16*this.mu,16*(e-16*h)*this.mu,16*(f-16*g)*this.mu,16*this.mu,16*this.mu),this.txtNeedUpdate[c]=1},paintMap:function(a,b,c,e){if(tilesData){a&&(this.mapSize=a);c?(this.treeValue=
[],this.clearAllTrees(),this.isWithHeight&&(this.heightData=this.generateHeight())):(this.tempBuildingLayers=[],this.tempHouseLayers=[]);if(0===this.miniCanvas.length)for(var f=0;f<this.nlayers;f++)this.miniCanvas[f]=document.createElement("canvas"),this.miniCanvas[f].width=this.miniCanvas[f].height=256*this.mu,this.miniCtx[f]=this.miniCanvas[f].getContext("2d"),this.isWithNormal&&(this.miniCanvasN[f]=document.createElement("canvas"),this.miniCanvasN[f].width=this.miniCanvasN[f].height=256*this.mu,
this.miniCtxN[f]=this.miniCanvasN[f].getContext("2d")),this.txtNeedUpdate[f]=0;a=!1;e=this.mapSize[1];for(var h,g,l,k,p=tilesData.length,q,n,m=0;e--;)for(h=this.mapSize[0];h--;)if(f=Math.floor(e/16),q=Math.floor(h/16),n=q+8*f,p--,g=tilesData[p],c&&(1<g&&5>g&&this.isWithHeight&&(this.heightData[this.findHeightId(h,e)]=-0.1),5<g&&21>g&&this.isWithHeight&&(this.heightData[this.findHeightId(h,e)]*=0.5),20<g&&44>g&&(this.isWithHeight&&(m=this.heightData[this.findHeightId(h,e)]-0.1),l=Math.floor(4*Math.random()),
36<=g&&(l+=4),c&&this.addTree(h,m,e,l,n),g=this.isWithTree?21+l:29+l,this.treeValue[p]=g)),this.fullRedraw&&20<g&&44>g&&(g=this.treeValue[p]),l=g%32*16,k=16*Math.floor(g/32),c||this.fullRedraw)this.miniCtx[n].drawImage(this.imageSrc,l*this.mu,k*this.mu,16*this.mu,16*this.mu,16*(h-16*q)*this.mu,16*(e-16*f)*this.mu,16*this.mu,16*this.mu),this.isWithNormal&&this.miniCtxN[n].drawImage(this.ground_n,l*this.mu,k*this.mu,16*this.mu,16*this.mu,16*(h-16*q)*this.mu,16*(e-16*f)*this.mu,16*this.mu,16*this.mu);
else if(h===this.forceUpdate.x&&e===this.forceUpdate.y&&(a=!0,this.forceUpdate.x=-1,this.forceUpdate.y=-1),43<g&&240>g||a)a&&(a=!1,20<g&&44>g&&(k=l=0)),this.miniCtx[n].drawImage(this.imageSrc,l*this.mu,k*this.mu,16*this.mu,16*this.mu,16*(h-16*q)*this.mu,16*(e-16*f)*this.mu,16*this.mu,16*this.mu),this.isWithNormal&&this.miniCtxN[n].drawImage(this.ground_n,l*this.mu,k*this.mu,16*this.mu,16*this.mu,16*(h-16*q)*this.mu,16*(e-16*f)*this.mu,16*this.mu,16*this.mu),this.txtNeedUpdate[n]=1;else if(240<g||
0==g)if(248<g&&261>g||0==g){if(this.houseLists[n])for(f=this.houseLists[n].length;f--;)q=this.houseLists[n][f],q[0]===h&&q[2]===e&&q[3]!==g&&(this.houseLists[n][f][3]=g,this.tempHouseLayers[n]=1)}else if(this.buildingLists[n])for(f=this.buildingLists[n].length;f--;)q=this.buildingLists[n][f],q[0]===h&&q[2]===e&&q[3]!==g&&(this.buildingLists[n][f][3]=g,this.tempBuildingLayers[n]=1);if(c)this.updateTerrain(b),this.populateTree();else{for(f=this.nlayers;f--;)if(this.txtNeedUpdate[f]||this.fullRedraw)this.terrainTxt[f].needsUpdate=
!0,this.txtNeedUpdate[f]=0;for(f=this.tempHouseLayers.length;f--;)1===this.tempHouseLayers[f]&&this.rebuildHouseLayer(f);for(f=this.tempBuildingLayers.length;f--;)1===this.tempBuildingLayers[f]&&this.rebuildBuildingLayer(f)}this.fullRedraw&&(this.fullRedraw=!1)}},moveSprite:function(){if(spriteData)for(var a=spriteData.length,b=new THREE.Vector3,c=0,e=0;a--;){var f=spriteData[a],c=f[1],e=f[0];b.x=Math.round((f[2]-8)/16);b.z=Math.round((f[3]-8)/16);b.y=0;this.isWithHeight&&(b.y=this.heightData[this.findHeightId(b.x,
