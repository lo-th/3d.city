...
Micro.SpriteManager.prototype = {
    ...
    moveObjects : function() {
        let messageManager = this.sim.messageManager;
        let disasterManager = this.sim.disasterManager;
        let blockMaps = this.sim.blockMaps;
    ...
    makeTornado : function(messageManager) {
        ...
        let x = Random.getRandom(Micro.worldToPix(this.map.width) - 800) + 400;
        let y = Random.getRandom(Micro.worldToPix(this.map.height) - 200) + 100;
    ...
    getBoatDistance : function(x, y) {
        let dist = 99999;
        let pixelX = Micro.worldToPix(x) + 8;
        let pixelY = Micro.worldToPix(y) + 8;
        let sprite;
    ...
    makeMonster : function(messageManager) {
        ...
        let done = 0;
        for (let i = 0; i < 300; i++)  {
            let x = Random.getRandom(this.map.width - 20) + 10;
            let y = Random.getRandom(this.map.height - 10) + 5;
